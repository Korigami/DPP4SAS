...
%macro size(set);
		%local rc k;
        %let rc = %sysfunc(
			dosubl('data _null_; set &set end=end;
					if end then call symputx("k",_N_);
					run;')
		);
        &k
%mend;
...
%macro compute_L(K, L);
	...
	proc iml;
		...
		r = round(trace(ginv(K)*K));
		if r = no_of_rows then 
...
%macro matrix_K(size, rank, random_state = 0);
proc iml;
	call streaminit(&random_state.);
	call streamrewind;
	x = j(&rank.*&size.,1);
